...
class Project:
    ...
    def to_model(self) -> "ProjectModel":
        return ProjectModel(
            name=self.name, folder_path=self.folder_path, created_at=self.created_at, updated_at=self.updated_at
        )
...
class Chunk:
    ...
    def from_model(cls, model: "ChunkModel") -> "Chunk":
        ...
        document_id_str = (
            str(model.document_id.to_ref().id) if hasattr(model.document_id, "to_ref") else str(model.document_id)
        )
...
class RiskType:
    ...
    def to_model(self) -> "RiskTypeModel":
        return RiskTypeModel(risk_type=self.risk_type, name=self.name, description=self.description, weight=self.weight)
...
class SourceDocument:
    ...
    def from_model(cls, model: "SourceDocumentModel") -> "SourceDocument":
        # Extract ObjectId from DBRef for Link fields
        project_id_str = (
            str(model.project_id.to_ref().id) if hasattr(model.project_id, "to_ref") else str(model.project_id)
        )
...
class EvidenceRating:
    ...
    def from_model(cls, model) -> "EvidenceRating":
        # Extract ObjectId from DBRef for Link fields
        risk_type_id_str = (
            str(model.risk_type_id.to_ref().id) if hasattr(model.risk_type_id, "to_ref") else str(model.risk_type_id)
        )
        risk_dimension_spec_id_str = (
            str(model.risk_dimension_spec_id.to_ref().id)
            if hasattr(model.risk_dimension_spec_id, "to_ref")
            else str(model.risk_dimension_spec_id)
        )
...
class Evidence:
    ...
    def from_model(cls, model) -> "Evidence":
        ...
        evidence_ratings_str = [
            str(rating_id.to_ref().id if hasattr(rating_id, "to_ref") else rating_id)
            for rating_id in model.evidence_ratings
        ]
...
class RiskAssessment:
    ...
    def from_model(cls, model) -> "RiskAssessment":
        ...
        evidence_ids_str = [
            str(evidence_id.to_ref().id if hasattr(evidence_id, "to_ref") else evidence_id)
...
class ProjectScore:
    ...
    def from_model(cls, model) -> "ProjectScore":
        ...
        risk_scores_str = [
            str(risk_id.to_ref().id if hasattr(risk_id, "to_ref") else risk_id) for risk_id in model.risk_scores
        ]
...
class ProcessingState:
    ...
    document_id: str | None = None  # For chunking stage
    chunk_id: str | None = None  # For evidence_extraction stage
    risk_type_id: str | None = None  # For evidence_extraction, risk_assessment stages
    ...
    def from_model(cls, model) -> "ProcessingState":
        ...
        document_id_str = (
            str(model.document_id.to_ref().id)
            if model.document_id and hasattr(model.document_id, "to_ref")
            else (str(model.document_id) if model.document_id else None)
        )
        chunk_id_str = (
            str(model.chunk_id.to_ref().id)
            if model.chunk_id and hasattr(model.chunk_id, "to_ref")
            else (str(model.chunk_id) if model.chunk_id else None)
        )
        risk_type_id_str = (
            str(model.risk_type_id.to_ref().id)
            if model.risk_type_id and hasattr(model.risk_type_id, "to_ref")
            else (str(model.risk_type_id) if model.risk_type_id else None)
        )
