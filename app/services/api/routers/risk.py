...
from app.services.api.schemas.risk_schemas import ProjectRiskBreakdown, ProjectSummary, RiskBreakdownItem, TopEvidence
...
@router.get("/risk-assessments/{risk_assessment_id}/top-evidences", response_model=list[TopEvidence])
async def get_risk_assessment_top_evidences(risk_assessment_id: str):
    """Get top 10 evidences for a specific risk assessment ordered by score (highest first)."""
    risk_assessment = await RiskAssessmentRepository.get_by_id(risk_assessment_id)
    if not risk_assessment:
        raise HTTPException(status_code=404, detail=f"Risk assessment not found with ID: {risk_assessment_id}")

    # Get evidences for this specific risk assessment
    if not risk_assessment.evidence_ids:
        return []  # No evidences for this risk assessment

    evidences = await EvidenceRepository.get_by_ids(risk_assessment.evidence_ids)

    # Sort evidences by score and take top 10
    sorted_evidences = sorted(evidences, key=lambda e: e.score, reverse=True)[:10]
