...
from app.core.types import Chunk, Project, SourceDocument
...
async def save_chunks_to_database(document_id: str, chunk_texts: list[str]) -> list[Chunk]:
    ...
    for i, chunk_text in enumerate(chunk_texts):
        chunk = Chunk(document_id=document_id, chunk_index=i, content=chunk_text, size_characters=len(chunk_text))
        chunks.append(chunk)
...
async def process_document_chunks(project: Project, document: SourceDocument) -> int:
    ...
    is_completed = await ProcessingStateRepository.is_completed(
        stage="chunking", project_id=project.id, document_id=document.id
    )
    ...
    await ProcessingStateRepository.update_status(
        stage="chunking", project_id=project.id, document_id=document.id, status="in_progress"
    )
    ...
    try:
        ...
        await ProcessingStateRepository.update_status(
            stage="chunking", project_id=project.id, document_id=document.id, status="failed", error_message=str(e)
        )
...
async def process_project_chunks(project: Project) -> int:
    ...
    try:
        ...
        max_chunks_per_project = (
            int(os.getenv("DEV_MAX_CHUNKS_PER_PROJECT", "999")) if os.getenv("APP_MODE") == "DEV" else 999
        )
