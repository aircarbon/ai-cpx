...
class ProjectScoreRepository:
    @staticmethod
    async def create_project_score(
        project: Project, risk_assessments: list[RiskAssessment], total_score: float
    ) -> ProjectScore:
    ...
    @staticmethod
    async def get_by_project(project_id: str) -> ProjectScore | None:
    ...
    @staticmethod
    async def update_or_create_project_score(
        project: Project, risk_assessments: list[RiskAssessment], total_score: float
    ) -> ProjectScore:
        """Update existing project score or create new one."""
        existing_model = await ProjectScoreModel.find_one(ProjectScoreModel.project_id == PydanticObjectId(project.id))
    ...
    async def project_has_score(project_id: str) -> bool:
        ...
        existing = await ProjectScoreModel.find_one(ProjectScoreModel.project_id == PydanticObjectId(project_id))
        return existing is not None

    @staticmethod
    async def delete_by_project(project_id: str) -> bool:
        """Delete project score for a specific project."""
        result = await ProjectScoreModel.find(ProjectScoreModel.project_id == PydanticObjectId(project_id)).delete()
        return result.deleted_count > 0

    @staticmethod
    async def update_summary(project_id: str, summary: str) -> ProjectScore | None:
    ...
    @staticmethod
    async def update_summary_by_id(project_score_id: str, summary: str) -> ProjectScore | None:
